...
export class A2AClient {
   ...
   * @param accessTokenProvider Optional function to provide access tokens for authenticated requests.
   */
  constructor(
    baseUrl: string,
    ...
    // Bind fetch to preserve context if it's the native browser fetch
    this.fetchImpl = typeof fetchImpl === 'function' && fetchImpl.name === 'fetch'
  ...
  /**
   * Sets the access token provider function.
   * This allows updating the token provider after client initialization.
   * @param provider Function that returns a promise resolving to an access token
   */
  public setAccessTokenProvider(provider: (() => Promise<string>) | null): void {
  ...
  private async _makeHttpRequest<Req extends A2ARequest>(
    ...
    // Add Authorization header if access token provider is available
    if (this.accessTokenProvider) {
      try {
        ...
        console.error("Failed to acquire access token:", tokenError);
        // Continue without token - let the backend handle auth error
      }
    }
  ...
  private async _handleJsonResponse<Res extends JSONRPCResponse>( // Takes full Response type
    ...
    try {
      ...
      if (response.status === 401 && this.accessTokenProvider) {
        console.warn('[A2AClient] Received 401, attempting token refresh and retry');
        try {
          // Force refresh token by calling provider again
          const newToken = await this.accessTokenProvider();
          if (newToken) {
            // Token refreshed, will be used in next request automatically
            console.log('[A2AClient] Token refreshed successfully');
          }
        } catch (refreshError) {
          console.error('[A2AClient] Token refresh failed:', refreshError);
        }
        // Note: We don't retry here automatically to avoid infinite loops
  ...
  private async *_handleStreamingResponse<StreamRes extends JSONRPCResponse>( // Takes full Response type
    ...
    const reader = response.body
      ...
      .getReader();
    let buffer = "";
    const MAX_BUFFER_SIZE = 10 * 1024 * 1024; // 10 MB limit to prevent memory issues
    ...
    try {
      while (true) {
        ...
        if (done) {
          ...
          if (buffer.trim()) {
            console.warn(
              `SSE stream ended with partial data in buffer for method ${expectedMethod}: ${buffer.substring(0, 200)}`
            );
          }
          break;
        }
        ...
        if (buffer.length > MAX_BUFFER_SIZE) {
          console.error(`SSE buffer exceeded maximum size (${MAX_BUFFER_SIZE} bytes) for method ${expectedMethod}`);
          throw new Error(`Stream buffer overflow: exceeded ${MAX_BUFFER_SIZE / (1024 * 1024)} MB limit`);
        }
          ...
          if (message.startsWith("data: ")) {
            ...
            if (dataLine) {
              ...
              try {
                ...
                const truncatedData = dataLine.length > 500
                  ? `${dataLine.substring(0, 500)}... (truncated, total length: ${dataLine.length})`
                  : dataLine;
                ...
                // If it's an "Invalid string length" error, the message is too large
                if (errorMsg.includes("Invalid string length") || errorMsg.includes("string length")) {
                  console.error(`Message payload is too large (${dataLine.length} characters). Consider chunking on server side.`);
                  throw new Error(`Network error: Invalid string length - message payload too large (${(dataLine.length / 1024).toFixed(2)} KB)`);
                }
              }
            }
