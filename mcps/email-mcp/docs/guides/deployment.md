...
This guide covers deploying EmailMcp to Azure Container Apps using the included Terraform infrastructure and CLI tools.
...
docker build \
  --platform linux/amd64 \
  -t your-registry.azurecr.io/email-mcp:v1.0.0 \
  .
...
resource_group_name = "rg-email-mcp-prod"
location = "eastus"
environment = "prod"

# Container configuration
mcp_container_image = "myregistry.azurecr.io/email-mcp:v1.0.0"
...
terraform init \
  ...
  -backend-config="key=email-mcp.tfstate"
...
```bash
email-mcp-cli iac deploy [OPTIONS]
...
```bash
email-mcp-cli iac destroy [OPTIONS]
...
```bash
email-mcp-cli docker build [OPTIONS]
...
```bash
email-mcp-cli docker push [OPTIONS]
...
```bash
email-mcp-cli docker login [OPTIONS]
...
# Re-push image
email-mcp-cli docker push --tag latest --registry <registry-url>
```
...
```bash
# Deploy new version
email-mcp-cli docker build --tag v2.0.0
email-mcp-cli docker push --tag v2.0.0 --registry <registry>

# Update terraform.tfvars with new image
mcp_container_image = "registry.azurecr.io/email-mcp:v2.0.0"

# Deploy (creates new revision)
email-mcp-cli iac deploy --auto-approve
...
az consumption usage list \
  ...
  --end-date 2024-01-31 \
  --query "[?contains(instanceName, 'email-mcp')]"
```
