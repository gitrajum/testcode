...
def build(
    ...
    Examples:
        ...
        mobile-contract-agent-cli docker build

        # Build with custom tag
        mobile-contract-agent-cli docker build -t v1.0.0

        # Build and tag for ACR
        mobile-contract-agent-cli docker build -r myacr.azurecr.io -t v1.0.0

        # Build and push
        mobile-contract-agent-cli docker build -r myacr.azurecr.io -t v1.0.0 --push
    """
    project_root = Path(__file__).parent.parent.parent

    agent_name = "mobile-contract-agent"
    ...
    console.print(
        f"[green]✓[/green] Image built successfully: [bold]{full_image}[/bold]"
    )
...
def push(
    ...
    Examples:
        ...
        mobile-contract-agent-cli docker push -r myacr.azurecr.io

        # Push specific tag
        mobile-contract-agent-cli docker push -r myacr.azurecr.io -t v1.0.0
    """
    agent_name = "mobile-contract-agent"
    full_image = f"{registry}/{agent_name}:{tag}"
    ...
    if result.returncode == 0:
        console.print(
            f"[green]✓[/green] Image pushed successfully: [bold]{full_image}[/bold]"
        )
    else:
        console.print("[red]✗[/red] Push failed")
...
def login(
    ...
    Examples:
        # Login to ACR
        mobile-contract-agent-cli docker login -r myacr.azurecr.io
    """
