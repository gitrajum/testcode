...
def _get_terraform_dir(terraform_dir: Optional[Path], environment: str) -> Path:
    ...
    if terraform_dir is None:
        ...
        if terraform_dir.name not in valid_environments:
            ...
            if not terraform_dir.exists():
                console.print(
                    f"[red]✗[/red] Environment subdirectory not found: {terraform_dir}"
                )
                raise typer.Exit(1)
...
def _run_terraform_command(
    cmd: list, cwd: Path, dry_run: bool = False
...
def init(
    ...
    terraform_dir: Optional[Path] = typer.Option(
        None, "--dir", "-d", help="Terraform directory"
    ),
    ...
    Examples:
        ...
        {mobile-contract-agent}-cli infra init

        # Initialize with remote state
        {mobile-contract-agent}-cli infra init --state-storage mystatestorage --state-rg state-rg
    """
...
def plan(
    ...
    Examples:
        # Plan deployment
        {mobile-contract-agent}-cli infra plan -rg my-rg -ce my-env

        # Plan destruction
        {mobile-contract-agent}-cli infra plan -rg my-rg -ce my-env --destroy
    """
    terraform_dir = _get_terraform_dir(terraform_dir, environment)
    backend_config = _get_backend_config(
        state_storage_account,
...
def apply(
    ...
    auto_approve: bool = typer.Option(
        False, "--auto-approve", help="Skip approval prompt"
    ),
    dry_run: bool = typer.Option(False, "--dry-run"),
):
    ...
    Examples:
        # Deploy agent
        {mobile-contract-agent}-cli infra apply -rg my-rg -ce my-env

        # Deploy with remote state
        {mobile-contract-agent}-cli infra apply -rg my-rg -ce my-env --state-storage mystatestorage

        # Deploy without approval (CI/CD)
        {mobile-contract-agent}-cli infra apply -rg my-rg -ce my-env --auto-approve
    """
    terraform_dir = _get_terraform_dir(terraform_dir, environment)
    backend_config = _get_backend_config(
        state_storage_account,
...
def destroy(
    ...
    Examples:
        ...
        {mobile-contract-agent}-cli infra destroy -rg my-rg -ce my-env

        # Destroy without approval
        {mobile-contract-agent}-cli infra destroy -rg my-rg -ce my-env --auto-approve
    """
    terraform_dir = _get_terraform_dir(terraform_dir, environment)
    backend_config = _get_backend_config(
        state_storage_account,
...
def validate(
    ...
    Examples:
        ...
        {mobile-contract-agent}-cli infra validate

        # Validate production
        {mobile-contract-agent}-cli infra validate --env prod
    """
    terraform_dir = _get_terraform_dir(terraform_dir, environment)

    console.print(
        f"[cyan]Validating Terraform configuration for {environment}...[/cyan]"
    )
...
def output(
    ...
    Examples:
        # Show all outputs
        {mobile-contract-agent}-cli infra output

        # Show specific output
        {mobile-contract-agent}-cli infra output container_app_url

        # JSON format
        {mobile-contract-agent}-cli infra output --json
    """
    terraform_dir = _get_terraform_dir(terraform_dir, environment)
