...
A AI agent based on the phased-orchestrator template

Built with the [agentic-ai-sdk](https://github.com/bayer-int/agentic_ai_sdk) library.

---

## 🌱 If you are new: how this project works

This service is the **backend brain** of the application.

- The **UI** sends a user message/request to this agent service.
- This agent uses **AgenticAI SDK** to decide how to process the request.
- It can call LLMs (Azure OpenAI), tools, and memory (Cosmos DB).
- It returns a response back to the UI.

### System in one sentence

**UI → MobileContractAgent → AgenticAI SDK + Azure services → Response to UI**

### Request lifecycle (simple flow)

1. User asks something in the UI.
2. UI sends the request to this backend (usually on port `8000`).
3. Authentication middleware validates user identity (if enabled).
4. Agent logic runs (prompting, tool usage, memory access).
5. The backend sends the final answer/event stream to the UI.
6. Logs and traces are emitted for debugging/monitoring.

### What each key file/folder is for

- `a2a_server.py` → API/server entry point.
- `configs/config.yaml` → local runtime configuration.
- `tools/` → custom tools the agent can call.
- `cli/commands/iac.py` → Terraform helper commands.
- `terraform/` → infrastructure code for cloud deployment.
- `.env.development` → local secrets and environment variables.

### First 30 minutes learning path (recommended)

1. Run the service locally with `agenticai dev .`.
2. Open and read `a2a_server.py` to see request entry points.
3. Read `configs/config.yaml` to understand model/tool wiring.
4. Check `tools/example_tool.py` to see how tools are added.
5. Send one test request and follow logs end-to-end.

### How to think while debugging

When something fails, check in this order:

1. **Server up?** (port/bind issues)
2. **Env variables set?** (`.env.development`)
3. **Auth enabled?** (token/middleware mismatch)
4. **LLM credentials valid?** (Azure OpenAI settings)
5. **Memory reachable?** (Cosmos DB endpoint/key)
6. **Tool code error?** (inside `tools/`)
...
mobile-contract-agent-cli iac

# Stop at plan to review changes
mobile-contract-agent-cli iac --last-step plan

# Only initialize
mobile-contract-agent-cli iac --last-step init

# Auto-approve for CI/CD
mobile-contract-agent-cli iac --auto-approve

# Use custom variables file
mobile-contract-agent-cli iac --var-file prod.tfvars

# Destroy infrastructure
mobile-contract-agent-cli iac --destroy --auto-approve

# Dry run (show commands without executing)
mobile-contract-agent-cli iac --dry-run
```
...
docker build -t mobile-contract-agent:latest .
...
- **Project Workflow (this folder)**: [WORKFLOW.md](./WORKFLOW.md)
...
**Version:** 0.1.0
**Author:** Jeel Soni <jeel.soni@bayer.com>
